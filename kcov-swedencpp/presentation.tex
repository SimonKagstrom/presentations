...
\begin{frame}
  ...
  \end{itemize}
  \includegraphics<1>[width=\linewidth]{sq_skull_jarring}
\end{frame}
...
\begin{frame}{Kcov overview}
  ...
  \note{
    ...
    retaining the userspace functionality. K therfore stands for
    kernel. The kernel functionality via kpropbes never worked very
    well though.
...
\begin{frame}{Python and Bash code coverage}
  \note{Judging by the number of reported github issues, Bash coverage collection is surprisingly popular. Or the kcov implementation is just buggy!}
  \begin{itemize}
